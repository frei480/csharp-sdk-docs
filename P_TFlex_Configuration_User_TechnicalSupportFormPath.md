

Руководство по T-FLEX CAD Open API

# UserTechnicalSupportFormPath - свойство  
  **Пространство имён:** [TFlex.Configuration](N_TFlex_Configuration.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public string TechnicalSupportFormPath { get; set; }
```
```vb
Public Property TechnicalSupportFormPath As String
	Get
	Set
```
```cpp
public:
property String^ TechnicalSupportFormPath {
	String^ get ();
	void set (String^ value);
}
```


#### Значение свойства

[String](https://learn.microsoft.com/dotnet/api/system.string)

#### Ссылки

[User - ](T_TFlex_Configuration_User.md)

[TFlex.Configuration - пространство имён](N_TFlex_Configuration.md)