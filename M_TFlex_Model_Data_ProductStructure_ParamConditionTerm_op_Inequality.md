

Руководство по T-FLEX CAD Open API

# ParamConditionTermInequality - оператор  
  
---  
**Пространство имён:** [TFlex.Model.Data.ProductStructure](N_TFlex_Model_Data_ProductStructure.md)**Сборка:** TFlexAPIData (в TFlexAPIData.dll) Версия: 17.1.20.0 (17.1.20.0)

```csharp
public static bool operator !=(
	ParamConditionTerm left,
	ParamConditionTerm right
)
```
```vb
Public Shared Operator <> ( 
	left As ParamConditionTerm,
	right As ParamConditionTerm
) As Boolean
```
```cpp
public:
static bool operator !=(
	ParamConditionTerm^ left, 
	ParamConditionTerm^ right
)
```


#### Параметры

left [ParamConditionTerm](T_TFlex_Model_Data_ProductStructure_ParamConditionTerm.md)
    
right [ParamConditionTerm](T_TFlex_Model_Data_ProductStructure_ParamConditionTerm.md)
    

#### Возвращаемое значение

[Boolean](https://learn.microsoft.com/dotnet/api/system.boolean)

#### Ссылки

[ParamConditionTerm - ](T_TFlex_Model_Data_ProductStructure_ParamConditionTerm.md)

[TFlex.Model.Data.ProductStructure - пространство имён](N_TFlex_Model_Data_ProductStructure.md)