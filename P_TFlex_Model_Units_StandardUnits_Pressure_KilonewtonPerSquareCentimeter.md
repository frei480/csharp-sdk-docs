

Руководство по T-FLEX CAD Open API

# StandardUnitsPressureKilonewtonPerSquareCentimeter - свойство  
    
Килоньютон на квадратный сантиметр; кН/см²; kN/cm²; 

Тип - Давление

**Пространство имён:** [TFlex.Model.Units](N_TFlex_Model_Units.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public static Unit KilonewtonPerSquareCentimeter { get; }
```
```vb
Public Shared ReadOnly Property KilonewtonPerSquareCentimeter As Unit
	Get
```
```cpp
public:
static property Unit^ KilonewtonPerSquareCentimeter {
	Unit^ get ();
}
```


#### Значение свойства

[Unit](T_TFlex_Model_Units_Unit.md)

#### Ссылки

[StandardUnitsPressure - ](T_TFlex_Model_Units_StandardUnits_Pressure.md)

[TFlex.Model.Units - пространство имён](N_TFlex_Model_Units.md)