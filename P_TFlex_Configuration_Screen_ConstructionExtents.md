

Руководство по T-FLEX CAD Open API

# ScreenConstructionExtents - свойство  
  **Пространство имён:** [TFlex.Configuration](N_TFlex_Configuration.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public DoubleParameter ConstructionExtents { get; set; }
```
```vb
Public Property ConstructionExtents As DoubleParameter
	Get
	Set
```
```cpp
public:
property DoubleParameter^ ConstructionExtents {
	DoubleParameter^ get ();
	void set (DoubleParameter^ value);
}
```


#### Значение свойства

[DoubleParameter](T_TFlex_Configuration_DoubleParameter.md)

#### Ссылки

[Screen - ](T_TFlex_Configuration_Screen.md)

[TFlex.Configuration - пространство имён](N_TFlex_Configuration.md)