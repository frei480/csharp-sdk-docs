...
# Document3DGetBends - метод  
  
---  
  
Контейнер операций гибки

**Пространство имён:** [TFlex.Model.Model3D](N_TFlex_Model_Model3D.md)**Сборка:** TFlexAPI3D (в TFlexAPI3D.dll) Версия: 17.1.20.0

```csharp
public static ICollection<Bend> GetBends(
	this Document document
)
```
```vb
<ExtensionAttribute>
Public Shared Function GetBends ( 
	document As Document
) As ICollection(Of Bend)
```
```cpp
public:
[ExtensionAttribute]
static ICollection<Bend^>^ GetBends(
	Document^ document
)
```
...
[ICollection](https://learn.microsoft.com/dotnet/api/system.collections.generic.icollection-1)[Bend](T_TFlex_Model_Model3D_Bend.md)
