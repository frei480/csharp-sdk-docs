

Руководство по T-FLEX CAD Open API

# StartCloseStartupPageAfterOpenDocument - свойство  
  **Пространство имён:** [TFlex.Configuration](N_TFlex_Configuration.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public bool CloseStartupPageAfterOpenDocument { get; set; }
```
```vb
Public Property CloseStartupPageAfterOpenDocument As Boolean
	Get
	Set
```
```cpp
public:
property bool CloseStartupPageAfterOpenDocument {
	bool get ();
	void set (bool value);
}
```


#### Значение свойства

[Boolean](https://learn.microsoft.com/dotnet/api/system.boolean)

#### Ссылки

[Start - ](T_TFlex_Configuration_Start.md)

[TFlex.Configuration - пространство имён](N_TFlex_Configuration.md)