...
# LineDraggerOrigin - свойство  
    
Начальная точка, от которой измеряется перемещение
...
```csharp
public BasePoint3D Origin { get; set; }
```
```vb
Public Property Origin As BasePoint3D
	Get
	Set
```
```cpp
public:
property BasePoint3D^ Origin {
	BasePoint3D^ get ();
