...
# SpecificationBOMHiddenRecsColor - свойство  
  **Пространство имён:** [TFlex.Configuration](N_TFlex_Configuration.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public int BOMHiddenRecsColor { get; set; }
```
```vb
Public Property BOMHiddenRecsColor As Integer
	Get
	Set
```
```cpp
public:
property int BOMHiddenRecsColor {
	int get ();
	void set (int value);
}
```
