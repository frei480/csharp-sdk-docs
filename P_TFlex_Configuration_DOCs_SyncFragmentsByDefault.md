...
# DOCsSyncFragmentsByDefault - свойство  
  **Пространство имён:** [TFlex.Configuration](N_TFlex_Configuration.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public bool SyncFragmentsByDefault { get; set; }
```
```vb
Public Property SyncFragmentsByDefault As Boolean
	Get
	Set
```
```cpp
public:
property bool SyncFragmentsByDefault {
	bool get ();
	void set (bool value);
}
```
