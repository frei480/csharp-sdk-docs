

Руководство по T-FLEX CAD Open API

# GraphicsRGBColor - свойство  
  
---  
  
Цвет, которым производится вывод, в виде компонент красного, зелёного и синего

**Пространство имён:** [TFlex.Drawing](N_TFlex_Drawing.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public int RGBColor { get; set; }
```
```vb
Public Property RGBColor As Integer
	Get
	Set
```
```cpp
public:
property int RGBColor {
	int get ();
	void set (int value);
}
```


#### Значение свойства

[Int32](https://learn.microsoft.com/dotnet/api/system.int32)

#### Ссылки

[Graphics - ](T_TFlex_Drawing_Graphics.md)

[TFlex.Drawing - пространство имён](N_TFlex_Drawing.md)