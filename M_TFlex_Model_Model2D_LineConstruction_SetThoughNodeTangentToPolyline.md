

Руководство по T-FLEX CAD Open API

# LineConstructionSetThoughNodeTangentToPolyline - метод  
  
---  
  
Прямая, проходящая через узел, касательная к сплайну или другой полилинии

**Пространство имён:** [TFlex.Model.Model2D](N_TFlex_Model_Model2D.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public void SetThoughNodeTangentToPolyline(
	Node srcNode,
	Construction srcPolyline,
	int variant
)
```
```vb
Public Sub SetThoughNodeTangentToPolyline ( 
	srcNode As Node,
	srcPolyline As Construction,
	variant As Integer
)
```
```cpp
public:
void SetThoughNodeTangentToPolyline(
	Node^ srcNode, 
	Construction^ srcPolyline, 
	int variant
)
```


#### Параметры

srcNode [Node](T_TFlex_Model_Model2D_Node.md)
    Исходный узел
srcPolyline [Construction](T_TFlex_Model_Model2D_Construction.md)
    Исходная полилиния
variant [Int32](https://learn.microsoft.com/dotnet/api/system.int32)
    Номер варианта прямой

#### Ссылки

[LineConstruction - ](T_TFlex_Model_Model2D_LineConstruction.md)

[TFlex.Model.Model2D - пространство имён](N_TFlex_Model_Model2D.md)