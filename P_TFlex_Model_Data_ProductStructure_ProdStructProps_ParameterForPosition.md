

Руководство по T-FLEX CAD Open API

# ProdStructPropsParameterForPosition - свойство  
  
---  
**Пространство имён:** [TFlex.Model.Data.ProductStructure](N_TFlex_Model_Data_ProductStructure.md)**Сборка:** TFlexAPIData (в TFlexAPIData.dll) Версия: 17.1.20.0 (17.1.20.0)

```csharp
public Guid ParameterForPosition { get; set; }
```
```vb
Public Property ParameterForPosition As Guid
	Get
	Set
```
```cpp
public:
property Guid ParameterForPosition {
	Guid get ();
	void set (Guid value);
}
```


#### Значение свойства

[Guid](https://learn.microsoft.com/dotnet/api/system.guid)

#### Ссылки

[ProdStructProps - ](T_TFlex_Model_Data_ProductStructure_ProdStructProps.md)

[TFlex.Model.Data.ProductStructure - пространство имён](N_TFlex_Model_Data_ProductStructure.md)