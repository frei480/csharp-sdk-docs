

Руководство по T-FLEX CAD Open API

# Settings3DSelectTransparentFacesByEdges - свойство  
  
---  
**Пространство имён:** [TFlex.Configuration](N_TFlex_Configuration.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public bool SelectTransparentFacesByEdges { get; set; }
```
```vb
Public Property SelectTransparentFacesByEdges As Boolean
	Get
	Set
```
```cpp
public:
property bool SelectTransparentFacesByEdges {
	bool get ();
	void set (bool value);
}
```


#### Значение свойства

[Boolean](https://learn.microsoft.com/dotnet/api/system.boolean)

#### Ссылки

[Settings3D - ](T_TFlex_Configuration_Settings3D.md)

[TFlex.Configuration - пространство имён](N_TFlex_Configuration.md)