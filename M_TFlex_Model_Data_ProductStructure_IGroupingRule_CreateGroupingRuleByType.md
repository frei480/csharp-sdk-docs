

Руководство по T-FLEX CAD Open API

# IGroupingRuleCreateGroupingRuleByType - метод  
  
---  
**Пространство имён:** [TFlex.Model.Data.ProductStructure](N_TFlex_Model_Data_ProductStructure.md)**Сборка:** TFlexAPIData (в TFlexAPIData.dll) Версия: 17.1.20.0 (17.1.20.0)

```csharp
public static IGroupingRule CreateGroupingRuleByType(
	GroupingRuleType type
)
```
```vb
Public Shared Function CreateGroupingRuleByType ( 
	type As GroupingRuleType
) As IGroupingRule
```
```cpp
public:
static IGroupingRule^ CreateGroupingRuleByType(
	GroupingRuleType type
)
```


#### Параметры

type [GroupingRuleType](T_TFlex_Model_Data_ProductStructure_GroupingRuleType.md)
    

#### Возвращаемое значение

[IGroupingRule](T_TFlex_Model_Data_ProductStructure_IGroupingRule.md)

#### Ссылки

[IGroupingRule - ](T_TFlex_Model_Data_ProductStructure_IGroupingRule.md)

[TFlex.Model.Data.ProductStructure - пространство имён](N_TFlex_Model_Data_ProductStructure.md)