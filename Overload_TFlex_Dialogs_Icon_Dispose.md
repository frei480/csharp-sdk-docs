

Руководство по T-FLEX CAD Open API

# IconDispose - метод  
    
|  | Имя | Описание |
| --- | --- | --- |
|  | [Dispose](M_TFlex_Dialogs_Icon_Dispose.md) | Освобождает все ресурсы, используемые объектом [Icon](T_TFlex_Dialogs_Icon.md) |
|  | [Dispose(Boolean)](M_TFlex_Dialogs_Icon_Dispose_1.md) | Освобождает неуправляемые ресурсы, используемые объектом [Icon](T_TFlex_Dialogs_Icon.md), а при необходимости освобождает также управляемые ресурсы |
  
#### Ссылки

[Icon - ](T_TFlex_Dialogs_Icon.md)

[TFlex.Dialogs - пространство имён](N_TFlex_Dialogs.md)