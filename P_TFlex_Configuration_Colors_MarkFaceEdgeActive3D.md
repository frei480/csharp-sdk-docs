...
# ColorsMarkFaceEdgeActive3D - свойство  
  
---  
**Пространство имён:** [TFlex.Configuration](N_TFlex_Configuration.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public int MarkFaceEdgeActive3D { get; set; }
```
```vb
Public Property MarkFaceEdgeActive3D As Integer
	Get
	Set
```
```cpp
public:
property int MarkFaceEdgeActive3D {
	int get ();
	void set (int value);
}
```
