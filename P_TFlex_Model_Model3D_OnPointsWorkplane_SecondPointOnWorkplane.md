

Руководство по T-FLEX CAD Open API

# OnPointsWorkplaneSecondPointOnWorkplane - свойство  
    
Вторая точку, задающая положение рабочей плоскости

**Пространство имён:** [TFlex.Model.Model3D](N_TFlex_Model_Model3D.md)**Сборка:** TFlexAPI3D (в TFlexAPI3D.dll) Версия: 17.1.20.0

```csharp
public ModelPoint3D SecondPointOnWorkplane { get; set; }
```
```vb
Public Property SecondPointOnWorkplane As ModelPoint3D
	Get
	Set
```
```cpp
public:
property ModelPoint3D^ SecondPointOnWorkplane {
	ModelPoint3D^ get ();
	void set (ModelPoint3D^ value);
}
```


#### Значение свойства

[ModelPoint3D](T_TFlex_Model_Model3D_Geometry_ModelPoint3D.md)

Положение рабочей плоскости задаётся следующими способами : \- точкой и осью; \- двумя точками (возможно задание угла доворота); \- тремя точками; \- точкой и конусом или цилиндром; \- двумя точками и плоскостью (возможно задание угла доворота); \- двумя точками и направлением (возможно задание угла доворота); \- двумя точками и цилиндром, конусом или сферой;

#### Ссылки

[OnPointsWorkplane - ](T_TFlex_Model_Model3D_OnPointsWorkplane.md)

[TFlex.Model.Model3D - пространство имён](N_TFlex_Model_Model3D.md)