

Руководство по T-FLEX CAD Open API

# WireConnectionInfoShowDenotationBegin - свойство  
    
Видимость ПО первой точки

**Пространство имён:** [TFlex.Model.Circuits](N_TFlex_Model_Circuits.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public bool ShowDenotationBegin { get; set; }
```
```vb
Public Property ShowDenotationBegin As Boolean
	Get
	Set
```
```cpp
public:
property bool ShowDenotationBegin {
	bool get ();
	void set (bool value);
}
```


#### Значение свойства

[Boolean](https://learn.microsoft.com/dotnet/api/system.boolean)

#### Ссылки

[WireConnectionInfo - ](T_TFlex_Model_Circuits_WireConnectionInfo.md)

[TFlex.Model.Circuits - пространство имён](N_TFlex_Model_Circuits.md)