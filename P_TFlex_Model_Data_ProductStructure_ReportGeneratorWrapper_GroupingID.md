

Руководство по T-FLEX CAD Open API

# ReportGeneratorWrapperGroupingID - свойство  
  
---  
  
Идентификатор используемой группировки.

**Пространство имён:** [TFlex.Model.Data.ProductStructure](N_TFlex_Model_Data_ProductStructure.md)**Сборка:** TFlexAPIData (в TFlexAPIData.dll) Версия: 17.1.20.0 (17.1.20.0)

```csharp
public Guid GroupingID { get; set; }
```
```vb
Public Property GroupingID As Guid
	Get
	Set
```
```cpp
public:
property Guid GroupingID {
	Guid get ();
	void set (Guid value);
}
```


#### Значение свойства

[Guid](https://learn.microsoft.com/dotnet/api/system.guid)

Если не задан, то используется группировка по умолчанию.

#### Ссылки

[ReportGeneratorWrapper - ](T_TFlex_Model_Data_ProductStructure_ReportGeneratorWrapper.md)

[TFlex.Model.Data.ProductStructure - пространство имён](N_TFlex_Model_Data_ProductStructure.md)