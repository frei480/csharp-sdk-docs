

Руководство по T-FLEX CAD Open API

# LoftGetGuideVectorConditionCount - метод  
  
---  
  
Получить число векторных условий

**Пространство имён:** [TFlex.Model.Model3D](N_TFlex_Model_Model3D.md)**Сборка:** TFlexAPI3D (в TFlexAPI3D.dll) Версия: 17.1.20.0

```csharp
public int GetGuideVectorConditionCount(
	int guideIndex
)
```
```vb
Public Function GetGuideVectorConditionCount ( 
	guideIndex As Integer
) As Integer
```
```cpp
public:
int GetGuideVectorConditionCount(
	int guideIndex
)
```


#### Параметры

guideIndex [Int32](https://learn.microsoft.com/dotnet/api/system.int32)
    Индекс направляющей (начинается с нуля)

#### Возвращаемое значение

[Int32](https://learn.microsoft.com/dotnet/api/system.int32)Число векторных условий

#### Ссылки

[Loft - ](T_TFlex_Model_Model3D_Loft.md)

[TFlex.Model.Model3D - пространство имён](N_TFlex_Model_Model3D.md)