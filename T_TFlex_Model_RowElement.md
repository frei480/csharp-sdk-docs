...
|  | [IncludeInAssembly](P_TFlex_Model_RowElement_IncludeInAssembly.md) | Получить ячейку "Включать в отчёты/спецификации текущего документа" |
|  | [IncludeInDoc](P_TFlex_Model_RowElement_IncludeInDoc.md) | Получить ячейку "Включать в отчёты/спецификации текущего документа" |
...
|  | [ItemParameterDescriptor](P_TFlex_Model_RowElement_Item_1.md) |  |
|  | [LinkedObjects](P_TFlex_Model_RowElement_LinkedObjects.md) | Модельные объекты, связанные с элементом структуры изделия |
...
|  | [ParentRowElement](P_TFlex_Model_RowElement_ParentRowElement.md) | Родительский элемент |
...
|  | [Position](P_TFlex_Model_RowElement_Position.md) | Получить ячейку "Включать при вставке в сборку" |
|  | [ProductStructure](P_TFlex_Model_RowElement_ProductStructure.md) | Структура изделия, которой принадлежит элемент |
|  | [SourceFragment3DFirstLevel](P_TFlex_Model_RowElement_SourceFragment3DFirstLevel.md) | Фрагмент 3D, из которого поднят элемент. Если элемент не поднят из фрагмента или поднят из фрагмента 2D, то возвращается null. |
|  | [SourceFragmentFirstLevel](P_TFlex_Model_RowElement_SourceFragmentFirstLevel.md) | Фрагмент, из которого поднят элемент. Если элемент не поднят из фрагмента, то возвращается null. |
|  | [SourceFragmentPath](P_TFlex_Model_RowElement_SourceFragmentPath.md) | Путь к фрагменту, из которого поднят элемент. Если элемент не поднят из фрагмента, то возвращается null. |
|  | [SourceObject](P_TFlex_Model_RowElement_SourceObject.md) | Исходный модельный объект, по которому создана запись. Если элемент не собран по текущему документу, то возвращается null. |
|  | [SourceRowElementUID](P_TFlex_Model_RowElement_SourceRowElementUID.md) | Уникальный идентификатор элемента, по которому создан этот элемент. Если элемент не поднят из фрагмента, то возвращается System.Guid.Empty |
|  | [SourceRowElementUIDFirstLevel](P_TFlex_Model_RowElement_SourceRowElementUIDFirstLevel.md) | Уникальный идентификатор элемента, по которому создан этот элемент (первого уровня вложенности). Если элемент не поднят из фрагмента, то возвращается System.Guid.Empty |
|  | [UID](P_TFlex_Model_RowElement_UID.md) | Уникальный идентификатор элемента |
...
|  | [CreateChangesScope](M_TFlex_Model_RowElement_CreateChangesScope.md) | Создать область изменений. Запись не будет автоматически пересчитываться при изменениях в ячейках. Вместо этого она пересчитается когда будет вызван Dispose. |
...
|  | [GetCell(ParameterDescriptor)](M_TFlex_Model_RowElement_GetCell_1.md) | Получить ячейку элемента для колонки структуры изделия |
...
|  | [GetSourceFragmentIdChain](M_TFlex_Model_RowElement_GetSourceFragmentIdChain.md) | Список идентификаторов фрагментов, из которых поднят элемент. Если элемент не поднят из фрагмента, то возвращается пустой перечислитель. |
