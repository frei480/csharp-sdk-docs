

Руководство по T-FLEX CAD Open API

# EllipseConstructionSetCenterAndNodeAndLine - метод  
  
---  
  
Эллипс с центром, проходящий через узел и касательный к прямой

**Пространство имён:** [TFlex.Model.Model2D](N_TFlex_Model_Model2D.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public void SetCenterAndNodeAndLine(
	Node centerNode,
	Node srcNode,
	Construction srcLine
)
```
```vb
Public Sub SetCenterAndNodeAndLine ( 
	centerNode As Node,
	srcNode As Node,
	srcLine As Construction
)
```
```cpp
public:
void SetCenterAndNodeAndLine(
	Node^ centerNode, 
	Node^ srcNode, 
	Construction^ srcLine
)
```


#### Параметры

centerNode [Node](T_TFlex_Model_Model2D_Node.md)
    Центр эллипса
srcNode [Node](T_TFlex_Model_Model2D_Node.md)
    Узел, через который проходит эллипс
srcLine [Construction](T_TFlex_Model_Model2D_Construction.md)
    Прямая, которой касается эллипс

#### Ссылки

[EllipseConstruction - ](T_TFlex_Model_Model2D_EllipseConstruction.md)

[TFlex.Model.Model2D - пространство имён](N_TFlex_Model_Model2D.md)