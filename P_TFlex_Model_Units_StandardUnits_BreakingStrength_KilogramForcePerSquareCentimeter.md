

Руководство по T-FLEX CAD Open API

# StandardUnitsBreakingStrengthKilogramForcePerSquareCentimeter - свойство  
    
КГС на квадратный сантиметр; кгс/см²; kgf/cm²; 

Тип - Прочность на разрыв

**Пространство имён:** [TFlex.Model.Units](N_TFlex_Model_Units.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public static Unit KilogramForcePerSquareCentimeter { get; }
```
```vb
Public Shared ReadOnly Property KilogramForcePerSquareCentimeter As Unit
	Get
```
```cpp
public:
static property Unit^ KilogramForcePerSquareCentimeter {
	Unit^ get ();
}
```


#### Значение свойства

[Unit](T_TFlex_Model_Units_Unit.md)

#### Ссылки

[StandardUnitsBreakingStrength - ](T_TFlex_Model_Units_StandardUnits_BreakingStrength.md)

[TFlex.Model.Units - пространство имён](N_TFlex_Model_Units.md)