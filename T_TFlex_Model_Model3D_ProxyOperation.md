...
# ProxyOperation - класс  
    
Базовый класс для 3D операций внешнего приложения, встраиваемых в модель

[SystemObject](https://learn.microsoft.com/dotnet/api/system.object) [TFlex.ModelProxyObject](T_TFlex_Model_ProxyObject.md) [TFlex.Model.Model3DProxyObject3D](T_TFlex_Model_Model3D_ProxyObject3D.md) TFlex.Model.Model3DProxyOperation
...
|  | Имя | Описание |
| --- | --- | --- |
|  | [ProxyOperation](M_TFlex_Model_Model3D_ProxyOperation__ctor.md) | Инициализирует новый экземпляр класса ProxyOperation |
|  | [ProxyOperation(IntPtr)](M_TFlex_Model_Model3D_ProxyOperation__ctor_1.md) | Инициализирует новый экземпляр класса ProxyOperation |
  
|  | Имя | Описание |
| --- | --- | --- |
|  | [AttributesFromSource](P_TFlex_Model_Model3D_ProxyOperation_AttributesFromSource.md) | Значение свойства "Атрибуты с исходной операции" |
|  | [CanBeSuppressed](P_TFlex_Model_Model3D_ProxyOperation_CanBeSuppressed.md) | Доступно ли подавление операции |
...
|  | [Operation](P_TFlex_Model_Model3D_ProxyOperation_Operation.md) | Внешняя операция, в которую агрегируется 3D операция внешнего приложения |
...
|  | Имя | Описание |
| --- | --- | --- |
|  | [AddAxis](M_TFlex_Model_Model3D_ProxyOperation_AddAxis.md) | Добавить ось в геометрию |
|  | [AddDiagnosticsMessage](M_TFlex_Model_Model3D_ProxyOperation_AddDiagnosticsMessage.md) | Добавить диагностическое сообщение. Надо вызывать из MakeGeometry(Переопределяет [ProxyObjectAddDiagnosticsMessage(DiagnosticsMessage)](M_TFlex_Model_ProxyObject_AddDiagnosticsMessage.md)) |
|  | [AddDirection](M_TFlex_Model_Model3D_ProxyOperation_AddDirection.md) | Добавить направление в геометрию |
|  | [AddPlane](M_TFlex_Model_Model3D_ProxyOperation_AddPlane.md) | Добавить плоскость в геометрию |
|  | [AddPoint](M_TFlex_Model_Model3D_ProxyOperation_AddPoint.md) | Добавить точку в геометрию |
|  | [AddSolid](M_TFlex_Model_Model3D_ProxyOperation_AddSolid.md) | Добавить тело в список тел операции |
|  | [AddToScene](M_TFlex_Model_Model3D_ProxyOperation_AddToScene.md) | Прогрузка операции в сцену |
...
|  | [CanAddDiagnosticsMessage](M_TFlex_Model_Model3D_ProxyOperation_CanAddDiagnosticsMessage.md) | Можно ли добавить диагностическое сообщение.(Переопределяет [ProxyObjectCanAddDiagnosticsMessage](M_TFlex_Model_ProxyObject_CanAddDiagnosticsMessage.md)) |
...
|  | [IsVirtualBody](M_TFlex_Model_Model3D_ProxyOperation_IsVirtualBody.md) | Виртуальное тело в моделе, особенность представления тела операции в дереве модели (по аналогии с 3D фрагментом или массивом) |
|  | [MakeGeometry](M_TFlex_Model_Model3D_ProxyOperation_MakeGeometry.md) | Пересчитать объект. Сформировать список тел в операции. |
...
Виртуальные методы, которые предлагается перекрыть, предназначены для вызова внешней операцией, в которую экземпляр класса агрегируется. Эти методы вызываются системой автоматически и не должны вызываться приложением.
