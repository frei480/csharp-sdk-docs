...
# ScreenTransparentTextEdit - свойство  
  **Пространство имён:** [TFlex.Configuration](N_TFlex_Configuration.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public bool TransparentTextEdit { get; set; }
```
```vb
Public Property TransparentTextEdit As Boolean
	Get
	Set
```
```cpp
public:
property bool TransparentTextEdit {
	bool get ();
	void set (bool value);
}
```
