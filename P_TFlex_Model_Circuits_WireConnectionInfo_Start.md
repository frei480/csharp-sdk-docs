...
# WireConnectionInfoStart - свойство  
  
---  
  
Первая точка подключения (откуда)

**Пространство имён:** [TFlex.Model.Circuits](N_TFlex_Model_Circuits.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public ModelObject Start { get; set; }
```
```vb
Public Property Start As ModelObject
	Get
	Set
```
```cpp
public:
property ModelObject^ Start {
	ModelObject^ get ();
