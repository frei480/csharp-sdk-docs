

Руководство по T-FLEX CAD Open API

# StandardColorsPathMark3 - свойство  
    
Цвет пометки путей и профилей 3

**Пространство имён:** [TFlex.Drawing](N_TFlex_Drawing.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public static Color PathMark3 { get; }
```
```vb
Public Shared ReadOnly Property PathMark3 As Color
	Get
```
```cpp
public:
static property Color PathMark3 {
	Color get ();
}
```


#### Значение свойства

[Color](https://learn.microsoft.com/dotnet/api/system.drawing.color)

#### Ссылки

[StandardColors - ](T_TFlex_Drawing_StandardColors.md)

[TFlex.Drawing - пространство имён](N_TFlex_Drawing.md)