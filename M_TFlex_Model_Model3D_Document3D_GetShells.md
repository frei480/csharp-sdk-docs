...
```csharp
public static ICollection<Shell> GetShells(
	this Document document
)
```
```vb
<ExtensionAttribute>
Public Shared Function GetShells ( 
	document As Document
) As ICollection(Of Shell)
```
```cpp
public:
[ExtensionAttribute]
static ICollection<Shell^>^ GetShells(
	Document^ document
)
```
...
[ICollection](https://learn.microsoft.com/dotnet/api/system.collections.generic.icollection-1)[Shell](T_TFlex_Model_Model3D_Shell.md)
