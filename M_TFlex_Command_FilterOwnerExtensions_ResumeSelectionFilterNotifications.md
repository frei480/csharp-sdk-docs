...
# FilterOwnerExtensionsResumeSelectionFilterNotifications - метод  
    
Закончить настройку панели фильтров

**Пространство имён:** [TFlex.Command](N_TFlex_Command.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public static void ResumeSelectionFilterNotifications(
	this IFilterOwner owner
)
```
```vb
<ExtensionAttribute>
Public Shared Sub ResumeSelectionFilterNotifications ( 
...
public:
[ExtensionAttribute]
static void ResumeSelectionFilterNotifications(
	IFilterOwner^ owner
)
```
...
Возобновляет перерисовку панели
