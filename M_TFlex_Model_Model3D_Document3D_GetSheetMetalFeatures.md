...
# Document3DGetSheetMetalFeatures - метод  
    
Контейнер операций выштамповки

**Пространство имён:** [TFlex.Model.Model3D](N_TFlex_Model_Model3D.md)**Сборка:** TFlexAPI3D (в TFlexAPI3D.dll) Версия: 17.1.20.0

```csharp
public static ICollection<SheetMetalFeature> GetSheetMetalFeatures(
	this Document document
)
```
```vb
<ExtensionAttribute>
Public Shared Function GetSheetMetalFeatures ( 
	document As Document
) As ICollection(Of SheetMetalFeature)
```
```cpp
public:
[ExtensionAttribute]
static ICollection<SheetMetalFeature^>^ GetSheetMetalFeatures(
	Document^ document
)
```
...
[ICollection](https://learn.microsoft.com/dotnet/api/system.collections.generic.icollection-1)[SheetMetalFeature](T_TFlex_Model_Model3D_SheetMetalFeature.md)
