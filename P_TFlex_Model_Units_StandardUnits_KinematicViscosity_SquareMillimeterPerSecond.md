

Руководство по T-FLEX CAD Open API

# StandardUnitsKinematicViscositySquareMillimeterPerSecond - свойство  
  
---  
  
Квадратный миллиметр в секунду; мм²/с; mm²/s; 

Тип - Кинематическая вязкость

**Пространство имён:** [TFlex.Model.Units](N_TFlex_Model_Units.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public static Unit SquareMillimeterPerSecond { get; }
```
```vb
Public Shared ReadOnly Property SquareMillimeterPerSecond As Unit
	Get
```
```cpp
public:
static property Unit^ SquareMillimeterPerSecond {
	Unit^ get ();
}
```


#### Значение свойства

[Unit](T_TFlex_Model_Units_Unit.md)

#### Ссылки

[StandardUnitsKinematicViscosity - ](T_TFlex_Model_Units_StandardUnits_KinematicViscosity.md)

[TFlex.Model.Units - пространство имён](N_TFlex_Model_Units.md)