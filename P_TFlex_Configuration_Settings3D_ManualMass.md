...
# Settings3DManualMass - свойство  
  **Пространство имён:** [TFlex.Configuration](N_TFlex_Configuration.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public DoubleParameter ManualMass { get; set; }
```
```vb
Public Property ManualMass As DoubleParameter
	Get
	Set
```
```cpp
public:
property DoubleParameter^ ManualMass {
	DoubleParameter^ get ();
