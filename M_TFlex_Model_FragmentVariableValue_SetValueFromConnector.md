

Руководство по T-FLEX CAD Open API

# FragmentVariableValueSetValueFromConnector - метод  
    
**Примечание: Данный API устарел.**

Установка значения переменной в соотвествии со значением этой переменной в коннекторе, использованном для привязки фрагмента

**Пространство имён:** [TFlex.Model](N_TFlex_Model.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
[ObsoleteAttribute("This method is obsolete and will be removed. Please use 'IsValueFromConnectors' property.")]
public void SetValueFromConnector()
```




#### Ссылки

[FragmentVariableValue - ](T_TFlex_Model_FragmentVariableValue.md)

[TFlex.Model - пространство имён](N_TFlex_Model.md)