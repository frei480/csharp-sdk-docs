

Руководство по T-FLEX CAD Open API

# IFilteringRuleCreateRuleByType - метод  
  **Пространство имён:** [TFlex.Model.Data.ProductStructure](N_TFlex_Model_Data_ProductStructure.md)**Сборка:** TFlexAPIData (в TFlexAPIData.dll) Версия: 17.1.20.0 (17.1.20.0)

```csharp
public static IFilteringRule CreateRuleByType(
	FilteringRuleType type
)
```
```vb
Public Shared Function CreateRuleByType ( 
	type As FilteringRuleType
) As IFilteringRule
```
```cpp
public:
static IFilteringRule^ CreateRuleByType(
	FilteringRuleType type
)
```


#### Параметры

type [FilteringRuleType](T_TFlex_Model_Data_ProductStructure_FilteringRuleType.md)
    

#### Возвращаемое значение

[IFilteringRule](T_TFlex_Model_Data_ProductStructure_IFilteringRule.md)

#### Ссылки

[IFilteringRule - ](T_TFlex_Model_Data_ProductStructure_IFilteringRule.md)

[TFlex.Model.Data.ProductStructure - пространство имён](N_TFlex_Model_Data_ProductStructure.md)