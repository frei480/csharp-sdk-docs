

Руководство по T-FLEX CAD Open API

# TableSetCursorPosition - метод  
  
---  
  
Установка положения курсора

**Пространство имён:** [TFlex.Model.Model2D](N_TFlex_Model_Model2D.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public void SetCursorPosition(
	uint cell,
	uint character
)
```
```vb
Public Sub SetCursorPosition ( 
	cell As UInteger,
	character As UInteger
)
```
```cpp
public:
void SetCursorPosition(
	unsigned int cell, 
	unsigned int character
)
```


#### Параметры

cell [UInt32](https://learn.microsoft.com/dotnet/api/system.uint32)
    Порядковый номер ячейки таблицы
character [UInt32](https://learn.microsoft.com/dotnet/api/system.uint32)
    Порядковый номер символа относительно начала ячейки

Параметры выделения фрагмента будут потеряны

#### Ссылки

[Table - ](T_TFlex_Model_Model2D_Table.md)

[TFlex.Model.Model2D - пространство имён](N_TFlex_Model_Model2D.md)