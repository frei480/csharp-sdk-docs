

Руководство по T-FLEX CAD Open API

# AltDimensionSeparatorTextAfter - свойство  
  **Пространство имён:** [TFlex.Configuration](N_TFlex_Configuration.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public CompoundStringParameter SeparatorTextAfter { get; set; }
```
```vb
Public Property SeparatorTextAfter As CompoundStringParameter
	Get
	Set
```
```cpp
public:
property CompoundStringParameter^ SeparatorTextAfter {
	CompoundStringParameter^ get ();
	void set (CompoundStringParameter^ value);
}
```


#### Значение свойства

[CompoundStringParameter](T_TFlex_Configuration_CompoundStringParameter.md)

#### Ссылки

[AltDimension - ](T_TFlex_Configuration_AltDimension.md)

[TFlex.Configuration - пространство имён](N_TFlex_Configuration.md)