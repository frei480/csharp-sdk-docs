

Руководство по T-FLEX CAD Open API

# StandardColorsNodeMark3Index - свойство  
  
---  
  
Цвет пометки 3D узлов 3

**Пространство имён:** [TFlex.Drawing](N_TFlex_Drawing.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public static int NodeMark3Index { get; }
```
```vb
Public Shared ReadOnly Property NodeMark3Index As Integer
	Get
```
```cpp
public:
static property int NodeMark3Index {
	int get ();
}
```


#### Значение свойства

[Int32](https://learn.microsoft.com/dotnet/api/system.int32)

#### Ссылки

[StandardColors - ](T_TFlex_Drawing_StandardColors.md)

[TFlex.Drawing - пространство имён](N_TFlex_Drawing.md)