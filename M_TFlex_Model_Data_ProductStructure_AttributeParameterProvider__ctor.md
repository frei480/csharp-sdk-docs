

Руководство по T-FLEX CAD Open API

# AttributeParameterProvider - конструктор  
    
Инициализирует новый экземпляр класса [AttributeParameterProvider](T_TFlex_Model_Data_ProductStructure_AttributeParameterProvider.md)

**Пространство имён:** [TFlex.Model.Data.ProductStructure](N_TFlex_Model_Data_ProductStructure.md)**Сборка:** TFlexAPIData (в TFlexAPIData.dll) Версия: 17.1.20.0 (17.1.20.0)

```csharp
public AttributeParameterProvider()
```
```vb
Public Sub New
```
```cpp
public:
AttributeParameterProvider()
```


#### Ссылки

[AttributeParameterProvider - ](T_TFlex_Model_Data_ProductStructure_AttributeParameterProvider.md)

[TFlex.Model.Data.ProductStructure - пространство имён](N_TFlex_Model_Data_ProductStructure.md)