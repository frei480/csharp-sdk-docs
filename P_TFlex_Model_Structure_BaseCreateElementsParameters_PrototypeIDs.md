...
# BaseCreateElementsParametersPrototypeIDs - свойство  
  **Пространство имён:** [TFlex.Model.Structure](N_TFlex_Model_Structure.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public IEnumerable<Guid> PrototypeIDs { get; set; }
```
```vb
Public Property PrototypeIDs As IEnumerable(Of Guid)
	Get
	Set
```
```cpp
public:
property IEnumerable<Guid>^ PrototypeIDs {
