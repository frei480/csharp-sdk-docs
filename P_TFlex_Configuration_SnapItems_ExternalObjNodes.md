...
# SnapItemsExternalObjNodes - свойство  
  **Пространство имён:** [TFlex.Configuration](N_TFlex_Configuration.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public SnapInfo ExternalObjNodes { get; }
```
```vb
Public ReadOnly Property ExternalObjNodes As SnapInfo
	Get
```
```cpp
public:
property SnapInfo^ ExternalObjNodes {
	SnapInfo^ get ();
}
```
