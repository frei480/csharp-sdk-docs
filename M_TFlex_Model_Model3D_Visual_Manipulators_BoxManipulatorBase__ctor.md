

Руководство по T-FLEX CAD Open API

# BoxManipulatorBase - конструктор  
  
---  
  
Инициализирует новый экземпляр класса [BoxManipulatorBase](T_TFlex_Model_Model3D_Visual_Manipulators_BoxManipulatorBase.md)

**Пространство имён:** [TFlex.Model.Model3D.Visual.Manipulators](N_TFlex_Model_Model3D_Visual_Manipulators.md)**Сборка:** TFlexAPI3D (в TFlexAPI3D.dll) Версия: 17.1.20.0

```csharp
protected BoxManipulatorBase(
	IApiManip* A_0
)
```
```vb
Visual Basic не поддерживает интерфейсы API, которые принимают или возвращают небезопасные типы.
```
```cpp
protected:
BoxManipulatorBase(
	IApiManip* A_0
)
```


#### Параметры

A_0 IApiManip*
    

#### Ссылки

[BoxManipulatorBase - ](T_TFlex_Model_Model3D_Visual_Manipulators_BoxManipulatorBase.md)

[TFlex.Model.Model3D.Visual.Manipulators - пространство имён](N_TFlex_Model_Model3D_Visual_Manipulators.md)