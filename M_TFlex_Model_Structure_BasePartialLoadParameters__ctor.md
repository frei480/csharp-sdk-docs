

Руководство по T-FLEX CAD Open API

# BasePartialLoadParameters - конструктор  
    
Инициализирует новый экземпляр класса [BasePartialLoadParameters](T_TFlex_Model_Structure_BasePartialLoadParameters.md)

**Пространство имён:** [TFlex.Model.Structure](N_TFlex_Model_Structure.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public BasePartialLoadParameters()
```
```vb
Public Sub New
```
```cpp
public:
BasePartialLoadParameters()
```


#### Ссылки

[BasePartialLoadParameters - ](T_TFlex_Model_Structure_BasePartialLoadParameters.md)

[TFlex.Model.Structure - пространство имён](N_TFlex_Model_Structure.md)