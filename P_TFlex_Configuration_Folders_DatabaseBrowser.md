...
# FoldersDatabaseBrowser - свойство  
  **Пространство имён:** [TFlex.Configuration](N_TFlex_Configuration.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public Collection DatabaseBrowser { get; }
```
```vb
Public ReadOnly Property DatabaseBrowser As Collection
	Get
```
```cpp
public:
property Collection^ DatabaseBrowser {
	Collection^ get ();
}
```
