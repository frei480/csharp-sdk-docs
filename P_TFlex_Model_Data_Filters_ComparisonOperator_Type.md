

Руководство по T-FLEX CAD Open API

# ComparisonOperatorType - свойство  
  
---  
  
Возвращает тип оператора

**Пространство имён:** [TFlex.Model.Data.Filters](N_TFlex_Model_Data_Filters.md)**Сборка:** TFlexAPIData (в TFlexAPIData.dll) Версия: 17.1.20.0 (17.1.20.0)

```csharp
public abstract ComparisonOperatorType Type { get; }
```
```vb
Public MustOverride ReadOnly Property Type As ComparisonOperatorType
	Get
```
```cpp
public:
virtual property ComparisonOperatorType Type {
	ComparisonOperatorType get () abstract;
}
```


#### Значение свойства

[ComparisonOperatorType](T_TFlex_Model_Data_Filters_ComparisonOperatorType.md)

#### Ссылки

[ComparisonOperator - ](T_TFlex_Model_Data_Filters_ComparisonOperator.md)

[TFlex.Model.Data.Filters - пространство имён](N_TFlex_Model_Data_Filters.md)