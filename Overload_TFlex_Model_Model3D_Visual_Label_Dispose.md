

Руководство по T-FLEX CAD Open API

# LabelDispose - метод  
    
|  | Имя | Описание |
| --- | --- | --- |
|  | [Dispose](M_TFlex_Model_Model3D_Visual_Label_Dispose.md) | Освобождает все ресурсы, используемые объектом [Label](T_TFlex_Model_Model3D_Visual_Label.md) |
|  | [Dispose(Boolean)](M_TFlex_Model_Model3D_Visual_Label_Dispose_1.md) | Освобождает неуправляемые ресурсы, используемые объектом [Label](T_TFlex_Model_Model3D_Visual_Label.md), а при необходимости освобождает также управляемые ресурсы |
  
#### Ссылки

[Label - ](T_TFlex_Model_Model3D_Visual_Label.md)

[TFlex.Model.Model3D.Visual - пространство имён](N_TFlex_Model_Model3D_Visual.md)