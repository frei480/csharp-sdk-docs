

Руководство по T-FLEX CAD Open API

# CircularDimensionSetDiametralDimensionType(DiametralDimensionType) - метод  
    
Установка размера, как диаметрального

**Пространство имён:** [TFlex.Model.Model2D](N_TFlex_Model_Model2D.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public void SetDiametralDimensionType(
	DiametralDimensionType type
)
```




#### Параметры

type [DiametralDimensionType](T_TFlex_Model_Model2D_DiametralDimensionType.md)
    Тип отрисовки диаметрального размера

#### Ссылки

[CircularDimension - ](T_TFlex_Model_Model2D_CircularDimension.md)

[SetDiametralDimensionType - перегрузка](Overload_TFlex_Model_Model2D_CircularDimension_SetDiametralDimensionType.md)

[TFlex.Model.Model2D - пространство имён](N_TFlex_Model_Model2D.md)