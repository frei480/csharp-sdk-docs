

Руководство по T-FLEX CAD Open API

# FormatParameterDataGenerator - конструктор  
    
Инициализирует новый экземпляр класса [FormatParameterDataGenerator](T_TFlex_Model_Data_ProductStructure_FormatParameterDataGenerator.md)

**Пространство имён:** [TFlex.Model.Data.ProductStructure](N_TFlex_Model_Data_ProductStructure.md)**Сборка:** TFlexAPIData (в TFlexAPIData.dll) Версия: 17.1.20.0 (17.1.20.0)

```csharp
public FormatParameterDataGenerator()
```
```vb
Public Sub New
```
```cpp
public:
FormatParameterDataGenerator()
```


#### Ссылки

[FormatParameterDataGenerator - ](T_TFlex_Model_Data_ProductStructure_FormatParameterDataGenerator.md)

[TFlex.Model.Data.ProductStructure - пространство имён](N_TFlex_Model_Data_ProductStructure.md)