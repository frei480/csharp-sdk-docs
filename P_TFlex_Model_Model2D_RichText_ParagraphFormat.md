

Руководство по T-FLEX CAD Open API

# RichTextParagraphFormat - свойство  
  
---  
  
Формат одного символа, на котором находится курсор, или символов, находящихся в выделенном фрагменте, в зависимости от состояния выделения

**Пространство имён:** [TFlex.Model.Model2D](N_TFlex_Model_Model2D.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public ParaFormat ParagraphFormat { get; set; }
```
```vb
Public Property ParagraphFormat As ParaFormat
	Get
	Set
```
```cpp
public:
property ParaFormat ParagraphFormat {
	ParaFormat get ();
	void set (ParaFormat value);
}
```


#### Значение свойства

[ParaFormat](T_TFlex_Model_Model2D_ParaFormat.md)

#### Ссылки

[RichText - ](T_TFlex_Model_Model2D_RichText.md)

[TFlex.Model.Model2D - пространство имён](N_TFlex_Model_Model2D.md)