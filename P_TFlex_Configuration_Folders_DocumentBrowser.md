

Руководство по T-FLEX CAD Open API

# FoldersDocumentBrowser - свойство  
  **Пространство имён:** [TFlex.Configuration](N_TFlex_Configuration.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public Collection DocumentBrowser { get; }
```
```vb
Public ReadOnly Property DocumentBrowser As Collection
	Get
```
```cpp
public:
property Collection^ DocumentBrowser {
	Collection^ get ();
}
```


#### Значение свойства

[Collection](T_TFlex_Configuration_Collection.md)

#### Ссылки

[Folders - ](T_TFlex_Configuration_Folders.md)

[TFlex.Configuration - пространство имён](N_TFlex_Configuration.md)