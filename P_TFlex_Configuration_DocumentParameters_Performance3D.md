

Руководство по T-FLEX CAD Open API

# DocumentParametersPerformance3D - свойство  
  
---  
**Пространство имён:** [TFlex.Configuration](N_TFlex_Configuration.md)**Сборка:** TFlexAPI (в TFlexAPI.dll) Версия: 17.1.20.0

```csharp
public Performance3D Performance3D { get; }
```
```vb
Public ReadOnly Property Performance3D As Performance3D
	Get
```
```cpp
public:
property Performance3D^ Performance3D {
	Performance3D^ get ();
}
```


#### Значение свойства

[Performance3D](T_TFlex_Configuration_Performance3D.md)

#### Ссылки

[DocumentParameters - ](T_TFlex_Configuration_DocumentParameters.md)

[TFlex.Configuration - пространство имён](N_TFlex_Configuration.md)