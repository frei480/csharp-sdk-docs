

Руководство по T-FLEX CAD Open API

# IParameterDataGeneratorCreateByType - метод  
  **Пространство имён:** [TFlex.Model.Data.ProductStructure](N_TFlex_Model_Data_ProductStructure.md)**Сборка:** TFlexAPIData (в TFlexAPIData.dll) Версия: 17.1.20.0 (17.1.20.0)

```csharp
public static IParameterDataGenerator CreateByType(
	ParamDataGenerType type
)
```
```vb
Public Shared Function CreateByType ( 
	type As ParamDataGenerType
) As IParameterDataGenerator
```
```cpp
public:
static IParameterDataGenerator^ CreateByType(
	ParamDataGenerType type
)
```


#### Параметры

type [ParamDataGenerType](T_TFlex_Model_Data_ProductStructure_ParamDataGenerType.md)
    

#### Возвращаемое значение

[IParameterDataGenerator](T_TFlex_Model_Data_ProductStructure_IParameterDataGenerator.md)

#### Ссылки

[IParameterDataGenerator - ](T_TFlex_Model_Data_ProductStructure_IParameterDataGenerator.md)

[TFlex.Model.Data.ProductStructure - пространство имён](N_TFlex_Model_Data_ProductStructure.md)