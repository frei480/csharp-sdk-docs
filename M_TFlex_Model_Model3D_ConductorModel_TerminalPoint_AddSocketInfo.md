

Руководство по T-FLEX CAD Open API

# TerminalPointAddSocketInfo - метод  
  **Пространство имён:** [TFlex.Model.Model3D.ConductorModel](N_TFlex_Model_Model3D_ConductorModel.md)**Сборка:** TFlexAPI3D (в TFlexAPI3D.dll) Версия: 17.1.20.0

```csharp
public SocketInfo AddSocketInfo(
	Element element,
	bool isInternal
)
```
```vb
Public Function AddSocketInfo ( 
	element As Element,
	isInternal As Boolean
) As SocketInfo
```
```cpp
public:
SocketInfo^ AddSocketInfo(
	Element^ element, 
	bool isInternal
)
```


#### Параметры

element [Element](T_TFlex_Model_Structure_Element.md)
    
isInternal [Boolean](https://learn.microsoft.com/dotnet/api/system.boolean)
    

#### Возвращаемое значение

[SocketInfo](T_TFlex_Model_Model3D_ConductorModel_SocketInfo.md)

#### Ссылки

[TerminalPoint - ](T_TFlex_Model_Model3D_ConductorModel_TerminalPoint.md)

[TFlex.Model.Model3D.ConductorModel - пространство имён](N_TFlex_Model_Model3D_ConductorModel.md)